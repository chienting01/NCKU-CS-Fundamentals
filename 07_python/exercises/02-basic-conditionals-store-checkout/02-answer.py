...
# 以下是看完題目實作後，紀錄再次修正與備註的結果

original_price = int(input("商品原價: "))
check_member = input("是否為會員(yes/no): ")
used_discount = input("是否使用折扣(yes/no): ")

# 錯誤寫法:
# 使用 bool(input(...)) 會讓幾乎所有非空字串都變成 True，輸入 Y 或 N 都會被當成 True。

used_discount_price = original_price 
#C 要先宣告型別，而且通常句尾會加分號。Python 不需要宣告型別，也不用分號。
# total_price = original_price 這一行沒必要
...
if check_member == "yes":
    used_discount_price = original_price*0.9

if used_discount == "yes":
    used_discount_price -= 50

if used_discount_price < 0:
    used_discount_price = 0

if used_discount_price >= 1000:
    fee = 0


final_price = used_discount_price + fee


print ("原價: ", original_price)
print ("折扣後金額: ", used_discount_price)
print ("運費: ", fee)
print ("應付總額: ", final_price) 
